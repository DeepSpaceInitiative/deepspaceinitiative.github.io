import Contentful, { Entry } from "contentful";
import { client } from "./client";

export interface TeamMember { 
  id: string,
  firstName: Contentful.EntryFields.Text,
  lastName: Contentful.EntryFields.Text,
  title: Contentful.EntryFields.Text,
  bio: Contentful.EntryFields.RichText,
  profileImage: Contentful.Asset,
}

export type Organization = {
  members: TeamMember[]
  whatDoWeDo: Contentful.EntryFields.RichText | null,
  whoAreWe: Contentful.EntryFields.RichText | null,
  whyJoinUs: Contentful.EntryFields.RichText | null,
}

type InternalOrganization = {
  members: Entry<TeamMember>[]
  whatDoWeDo: Contentful.EntryFields.RichText | null,
  whoAreWe: Contentful.EntryFields.RichText | null,
  whyJoinUs: Contentful.EntryFields.RichText | null,
}

export const getCoreTeamMembers = async (): Promise<Organization> => 
  client
    .getEntry<InternalOrganization>("7FQbpFmwuuzZztk6lNXKK9")
    .then(result => { return { members: result.fields.members.map(member => { return { id: member.sys.id, ...member.fields }}), whatDoWeDo: result.fields.whatDoWeDo, whoAreWe: result.fields.whoAreWe, whyJoinUs: result.fields.whyJoinUs}})


export const getAdvisors = async (): Promise<TeamMember[]> => 
  client
    .getEntry<InternalOrganization>("3zI2vZJfVCl1H9f2aXyhad")
    .then(result => result.fields.members.map(member => { return { id: member.sys.id, ...member.fields } }))

export const getSupervisors = async (): Promise<TeamMember[]> => 
  client
    .getEntry<InternalOrganization>("3Z3RZtAF02jDkdqmumny8h")
    .then(result => result.fields.members.map(member => { return { id: member.sys.id, ...member.fields } }))
  